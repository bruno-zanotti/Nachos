...
AddressSpace::AddressSpace(OpenFile *executable)
{
    ...
    for (unsigned i = 0; i < numPages; i++) {
        int c = mapTable -> Find();
        pageTable[i].virtualPage  = c;
          // For now, virtual page number = physical page number.
        pageTable[i].physicalPage = i;      //chequar c?
    ...
    for (unsigned i = 0; i < numPages; i++) {
        int c = pageTable[i].virtualPage;
        memset(mainMemory, c*PAGE_SIZE, PAGE_SIZE);
    }
